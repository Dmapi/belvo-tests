...
typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('d3@6.7.0/dist/d3.min.js')) :
typeof define === 'function' && define.amd ? define(['exports', 'd3@6.7.0/dist/d3.min.js'], factory) :
...
function createAxis(axis, scale, {ticks, tickSize, tickPadding, tickFormat}) {
  if (!scale.tickFormat && typeof tickFormat !== "function") {
    ...
    tickFormat = tickFormat === undefined ? (isTemporal(scale.domain()) ? formatIsoDate : string)
      : (typeof tickFormat === "string" ? (isTemporal(scale.domain()) ? d3.utcFormat : d3.format)
      : constant)(tickFormat);
  }
  return axis(scale)
...
function ScaleQ(key, scale, channels, {
  ...
  scheme,
  type,
  interpolate = registry.get(key) === color ? (range !== undefined ? d3.interpolateRgb : scheme !== undefined ? Scheme$1(scheme) : type === "cyclical" ? d3.interpolateRainbow : d3.interpolateTurbo) : round ? d3.interpolateRound : undefined,
  reverse,
  inset
}) {
...
function ScaleLog(key, channels, {base = 10, domain = inferLogDomain(channels), ...options}) {
  return ScaleQ(key, d3.scaleLog().base(base), channels, {domain, ...options});
}
...
function ScaleDiverging(key, channels, {
  ...
  range,
  scheme,
  interpolate = registry.get(key) === color ? (range !== undefined ? d3.interpolateRgb : scheme !== undefined ? Scheme$1(scheme) : d3.interpolateRdBu) : undefined,
  reverse
}) {
...
function inferZeroDomain(channels) {
  return [0, d3.max(channels, ({value}) => value === undefined ? value : d3.max(value))];
}
...
function ScaleOrdinal(key, channels, {
  ...
  range = registry.get(key) === color ? (scheme !== undefined ? Scheme(scheme)
    ...
    : d3.schemeTableau10) : undefined,
  ...options
}) {
  return ScaleO(d3.scaleOrdinal().unknown(undefined), channels, {range, ...options});
}
...
function bin(outputs, {inset, insetTop, insetRight, insetBottom, insetLeft, ...options} = {}) {
